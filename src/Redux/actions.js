 ...
 */
export const searchFAQ = (params) => async(dispatch) => {
    try {
 ...
 */
export const postReview = (details, review, uuid) => async(dispatch) => {

    const newLocationProf = await postReviewToLocation(details, review, uuid)

    dispatch({ type: "POST_REVIEW", payload: newLocationProf })
}
...
export const getLocationsRandomSearch = () => async (dispatch) => {
    try {
        
        const data = await api.fetchLocationsRandomSearch()
...
export const nextPage = (currPage) => async (dispatch) => {
    console.log("actions next page")
    dispatch({type: "NEXT_PAGE", payload: currPage++})
    
}

export const getLocationsByFavorites = (locationID) => async (dispatch) => {
    try {
